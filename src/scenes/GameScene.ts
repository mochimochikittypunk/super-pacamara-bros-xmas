...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    for (let x = 0; x < this.levelWidth; x += 64) {
      // ★ 穴（崖）を作る：特定の範囲だけブロックを作らない
      // 例: 2000〜2200, 3500〜3700 を穴にする
      if ((x > 2000 && x < 2200) || (x > 3500 && x < 3700)) {
        continue;
      }
    ...
    // 後半は、同じパターンを +2400 右にずらして配置
    const platformData = [
      ...platformPattern,
      ...platformPattern.map((p) => ({ x: p.x + 2400, y: p.y }))
    ];
    ...
    this.createEnemy(this.enemiesSlow, 500, height - 64, 40);
    this.createEnemy(this.enemiesFast, 800, height - 64, 80);
    this.createEnemy(this.enemiesSlow, 1200, height - 64, 40);
    this.createEnemy(this.enemiesFast, 1600, height - 64, 80);

    // 後半の敵（+2400 右にずらした位置、全て地面の高さ）
    this.createEnemy(this.enemiesSlow, 500 + 2400, height - 64, 40);
    this.createEnemy(this.enemiesFast, 800 + 2400, height - 64, 80);
  ...
  private createEnemy(
    ...
    const texture = speed > 50 ? "enemyFast" : "enemySlow";
  ...
  private updateEnemy(enemy: Phaser.Physics.Arcade.Sprite) {
    ...
    // この範囲（startX ± 80）で左右にパトロールさせる
    const leftLimit = startX - 80;
    const rightLimit = startX + 80;
